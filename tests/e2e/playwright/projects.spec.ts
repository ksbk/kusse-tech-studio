...
test.describe('Projects Showcase', () => {
  test('projects page loads and displays projects', async ({ page }) => {
    ...
    const projectItems = page.locator('.project, .project-item, .project-card, article');
    ...
    if (projectCount > 0) {
      ...
      const projectTitle = firstProject.locator('h2, h3, .project-title, .title');
  ...
  test('project filtering/categories work (if present)', async ({ page }) => {
    ...
    const filterButtons = page.locator('.filter, .category, .tag, button[data-filter]');
    const filterCount = await filterButtons.count();
    
    if (filterCount > 0) {
      const allProjects = page.locator('.project, .project-item, .project-card');
      ...
      const filteredProjects = page.locator('.project:visible, .project-item:visible, .project-card:visible');
  ...
  test('project images load correctly', async ({ page }) => {
    ...
    const projectImages = page.locator('.project img, .project-item img, .project-card img');
  ...
  test('projects page is responsive', async ({ page }) => {
    ...
    const projectsDesktop = page.locator('.project, .project-item, .project-card');
    const desktopCount = await projectsDesktop.count();
    
    // Test tablet view
    await page.setViewportSize({ width: 768, height: 1024 });
    await page.waitForTimeout(500); // Wait for responsive changes
    const projectsTablet = page.locator('.project:visible, .project-item:visible, .project-card:visible');
    const tabletCount = await projectsTablet.count();
    
    // Test mobile view
    await page.setViewportSize({ width: 375, height: 667 });
    await page.waitForTimeout(500);
    const projectsMobile = page.locator('.project:visible, .project-item:visible, .project-card:visible');
  ...
  test('project pagination works (if present)', async ({ page }) => {
    ...
    if (await pagination.count() > 0) {
      const nextButton = pagination.locator('a:has-text("Next"), a:has-text("›"), a:has-text("»"), .next');
