...
test.describe("Site Navigation", () => {
  ...
  test("mobile navigation menu works", async ({ page }) => {
    ...
    const mobileToggle = page.locator(
      '.mobile-menu-toggle, .hamburger, .menu-toggle, button:has-text("Menu")',
    );
    ...
    if ((await mobileToggle.count()) > 0) {
      ...
      if ((await mobileMenu.count()) > 0) {
        ...
        const aboutLink = mobileMenu.locator(
          'a[href*="/about"], a:has-text("About")',
        );
  ...
  test("navigation is keyboard accessible", async ({ page }) => {
    ...
    while (tagName !== "a" && attempts < 10) {
      ...
      if ((await focusedElement.count()) > 0) {
        tagName = await focusedElement.evaluate((el) =>
          el.tagName.toLowerCase(),
        );
      }
      attempts++;
