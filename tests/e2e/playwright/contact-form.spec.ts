...
test.describe('Contact Form', () => {
  ...
  test('successful form submission', async ({ page }) => {
    ...
    const successMessages = page.locator('.success, .alert-success, .flash-success');
    if (await successMessages.count() > 0) {
      await expect(successMessages.first()).toBeVisible();
      await expect(successMessages.first()).toContainText(/thank you|success|received/i);
    }
  });
  ...
  test('form fields accept correct input types', async ({ page }) => {
    ...
    // Test message field
    const testMessage = 'This is a longer test message to verify the textarea can handle multiple lines and longer content.';
  ...
  test('form is accessible', async ({ page }) => {
    ...
    await expect(page.locator('label[for="name"], label:has(input[name="name"])')).toBeVisible();
    await expect(page.locator('label[for="email"], label:has(input[name="email"])')).toBeVisible();
    await expect(page.locator('label[for="message"], label:has(textarea[name="message"])')).toBeVisible();
